...
p = Path('src/ratings/direct_measurement.mjs')
s = p.read_text()
...
def replace_once(old, new):
    ...
    n = s.count(old)
    if n != 1:
        raise SystemExit(f'expected exactly 1 match, got {n}: {old[:80]!r}')
    s = s.replace(old, new, 1)

marker = """export function measuredYearsFromDetail(detail, key) {
  if (!detail) return [];
  try {
