...
function splitCommunity(nameKey, oldCommunity) {
  const rows = xByName.get(nameKey) ?? [];
  const physicalRows = rows.filter(r => /CURRENT_REALWORLD_SPEED_PHYSICAL|PHYSICAL/i.test(String(r.disposition ?? r.lane ?? r.classification ?? r.evidence_lane ?? '')));
  const techniqueRows = rows.filter(r => /TECHNIQUE/i.test(String(r.disposition ?? r.lane ?? r.classification ?? r.evidence_lane ?? '')));
  const ratingRows = rows.filter(r => /POWERPRO_RATING|RATING/i.test(String(r.disposition ?? r.lane ?? r.classification ?? r.evidence_lane ?? '')));
  const compact = r => ({
    source_date: r.date ?? r.published_at ?? r.source_date ?? null,
    disposition: r.disposition ?? r.lane ?? r.classification ?? null,
    direction: r.direction ?? r.rating_direction ?? null,
    text: r.text ?? r.content ?? r.post_text ?? null,
