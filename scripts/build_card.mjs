...
//
// 2026-08-07 安全停止:
//   peak: 2020年以降は年度候補へUBR・位置別守備得点を接続済み。ただし2019年以前は
//         NPB Basement由来の走守得点が無く、全キャリアを同条件で比較できない。
//         歴史年を欠損0として扱わないため、一般CLIではまだ自動選定を停止する。
//   prime: 打撃は複数年合成だが走塁・守備等は窓末年を使う期間混在と、環境補正の
//          二重適用余地が残るため停止を継続する。
//   現時点では年度を明示して単年カードを作る。
//
...
if (MODE === 'peak' || MODE === 'prime') {
  console.error(
    ...
    + (MODE === 'peak'
      ? '理由: 2020+の走守得点は接続済みだが、2019年以前との全キャリア比較を同条件で行えない。'
      : '理由: primeは能力ごとの参照期間と環境補正・得能合成が未統一。')
  );
  process.exit(2);
}
if (!/^\d{4}$/.test(MODE)) {
