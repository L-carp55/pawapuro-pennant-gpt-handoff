...
//
// 2026-08-07安全修正:
//   - raw/DB pathを環境変数で差し替え可能にし、一時DBで非破壊監査できる。
//   - 打席ヘッダー・交代・牽制等の非投球行を最終守備プレーへ混ぜない。
//   - 守備負荷の「守備球数」も実投球行だけを数える。
//   - current gameの最終球数は当該プレーより後の情報を含むため、説明変数には使わない。
//
...
const events=[];
// season|game_id -> {season,date,game_id, players: Map(playerNorm -> actual-pitch count)}
...
for(const fn of files){
  ...
  for(let i=1;i<lines.length;i++){
    ...
    // 実投球だけから守備負荷を数える。非投球ヘッダーや牽制イベントは含めない。
  ...
  for(const r of lastPitch.values()){
    ...
    if(!pos || !r[c.fielder])continue; // 分母と分子を同じ「処理野手が特定できる打球」に限定
...
// 各選手×試合の実守備投球数を、当該試合より前に観測できる負荷コンテキストへ変換。
const workloadRows=[];
