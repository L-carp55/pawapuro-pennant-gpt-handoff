...
push('## 判定\n');
{
  ...
  push('### 結論: `NOT_IDENTIFIABLE`（現在のデータでは判定できない）\n');
  push('**現在のデータでは rolling holdout による NPB+ blend の採否を決められない。**\n');
  push('理由:\n');
  push('- 統計側は T-0198 の修理で `maxSeason = Y-1` へ**正しく制限できる**');
  push('- だが **NPB+ には過去年の snapshot が無い**。2026年に1回測った値しか存在せず、');
  push('  同じ時点へ制限することが**原理的にできない**');
  push('- したがって NPB+ を含む方式と含まない方式を同じ土俵で比べられない\n');
  push('この非対称を消す方法は現在のデータには無い。**判定は保留する。**\n');

  push('### 正式な control（漏れなし。以後の比較の基準線）\n');
  push('```text');
  push(`統計モデル単独（maxSeason=Y-1、漏れなし）   平均r ${f3(g('(c) NPB+blendなし'))}`);
  push(`  誤差(z単位) ${summary['(c) NPB+blendなし']?.wm?.toFixed(3) ?? '-'}   有効セル ${summary['(c) NPB+blendなし']?.cells ?? 0}`);
  push('```');
  push('これが**唯一、予測年以降の情報を含まない数字**であり、正式なcontrolとして保存する。');
  push('NPB+を含む方式（現行blend・NPB+単独）はこのcontrolと**同じ土俵にない**ため、');
  push('数値を並べても優劣の判定にはならない。\n');

  push('### 参考diagnostic（★採否の根拠に使わない）\n');
  push('以下は「未来情報を同程度与えるとどうなるか」を見るための診断であり、');
  push('**validation / acceptance criterion から外す**（2026-08-13 オーナー指示）。');
  push('`statOpen`＝統計側にも窓制限を掛けない版で、**予測年以降の情報を含む**。\n');
  const A = '統計・窓制限なし(参考)', B = '窓制限なし+NPB+blend(実運用条件)';
  const cw = cellWin(A, B);
  push('```text');
  push(`統計・窓制限なし(diagnostic)          ${f3(g(A))}`);
  push(`窓制限なし+NPB+blend(diagnostic)     ${f3(g(B))}`);
  push(`セル単位 ${cw.w}勝 ${cw.l}敗 ${cw.t}分`);
  push('```');
  push('★この 2 数値と勝敗は **(c) 採用の根拠にしてはならない**。両側とも漏れている。\n');
  push('以前この文書は同じ比較をもって「現行NPB+ blendは一貫した悪化要因」と結論していたが、');
  push('**撤回した**。限界節に「両側に窓の緩みがある」と書きながら、その比較を結論に使っていた。\n');
}
