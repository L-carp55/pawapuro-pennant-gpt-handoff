...
# A successful metadata fetch is not a positive finding.  Once the frozen target
# search is complete, zero usable directional/timed context is itself a measured
# bounded negative result, even if one or more fetches returned no comments.
measured_no_usable_context=bool(q) and usable==0 and timed==0
status=('PASS_BOUNDED_ACQUISITION_LIMITED_NEGATIVE_FINDING' if sem and (query_errors==len(q) or measured_no_usable_context or (fetch and fetch_fail==len(fetch) and usable==0)) else ('PASS_TARGETED_RESCUE_BOUNDED_VALIDATED' if sem else 'FAIL_SP102_TARGETED_RESCUE_QA'))
