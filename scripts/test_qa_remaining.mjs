...
//   CCは当初これを層別せずに測り「使えないと確定」と誤報告した。
{
  ...
  // ── §走塁材料 自作の走塁指標を走力へ（2026-08-05 オーナー承認）──────────
  // 単打での一塁→三塁など「余分に進めたか」を打球の位置とアウトカウントで揃えて測ったもの。
  // 翌年との一致 0.527 で、いま使っているUBR(0.447)より安定している。置き換えず足す。
  {
    const { speedComponents } = await import('../src/ratings/running.mjs');
    const { advanceOf } = await import('../src/ratings/baserunning_advance.mjs');
    ...
    const fast = speedComponents(line, { ...base, advance: 0.15, advanceChances: 40 }, 2.0, runNorm);
    const slow = speedComponents(line, { ...base, advance: -0.15, advanceChances: 40 }, 2.0, runNorm);

    t('§走塁材料-a 走塁が上手いと走力が上がる',
    ...
    t('§走塁材料-f 複数年を均す経路にも配線されている（片側だけ直す誤りの再発検知）',
      src.includes('advanceOf(') && src.includes('advance:'), 'durable_estimate.mjs に配線あり');

    // 実データで実際に効いているか
    if (db.prepare(`SELECT 1 FROM sqlite_master WHERE type='table' AND name='baserunning_advances'`).get()) {
      const a = advanceOf(db, '周東佑京', 2024), b2 = advanceOf(db, '山川穂高', 2024);
      t('§走塁材料-g 実データで俊足と鈍足の差が出る',
        a && b2 && a.value > b2.value, a && b2 ? `周東${a.value.toFixed(3)} > 山川${b2.value.toFixed(3)}` : '引けず');

      // ★機会の下限を25→20へ（2026-08-05）。20と25で翌年再現性はほぼ同じ(0.527 vs 0.533)なのに
      //   対象が55人→73人（2024年）へ3割増える。周東佑京（21機会）は25だと漏れていた
      //   （出塁後すぐ盗塁するので「単打で一塁から三塁」の場面が生じにくく、機会が少ない）。
      t('§走塁材料-h 機会の下限は20（周東のような俊足選手を漏らさない）',
        runNorm.advance?.min_chances === 20, `min_chances=${runNorm.advance?.min_chances}`);
      const shuto = advanceOf(db, '周東佑京', 2024);
      t('§走塁材料-i 周東佑京（21機会）が下限20で対象に入る（25だと漏れていた）',
        shuto && shuto.chances >= 20 && shuto.chances < 25,
        shuto ? `${shuto.chances}機会（下限20 ✓ / 旧下限25 ✗）` : '引けず');
      t('§走塁材料-j 21機会は走力の計算に実際に使われる',
        speedComponents(line, { ...base, advance: shuto?.value ?? 0.1, advanceChances: shuto?.chances ?? 21 }, 2.0, runNorm)
          .z.advance != null, `${shuto?.chances}機会で使用`);
    }
  }
  ...
  // ── §内野送球 内野手の送球（精度）の得能（2026-08-05・オーナー指示の要素切り分け）──
  {
    ...
    if (existsSync('outputs/derived/infield_throw_accuracy.json')) {
      ...
      t('§内野送球-d 内野手の送球が能力欄まで届く（証拠だけ出て能力が動かない状態の再発検知）',
